...
print("PESEL number verification. Version 4.3\nType your PESEL number below:")
pesel_input = input()
(month, day) = (0, 0)
...
def leap_year(n):
    if pesel_input_validation is True:
...
def pesel_input_sex(n):
    if (pesel_input_validation is True) and (date_input_validation is True):
        if n[9] in '02468':
...
beginning(pesel_input), leap_year(pesel_input), 
birth_date(month, day), pesel_input_sex(pesel_input), pesel_age(pesel_input)
